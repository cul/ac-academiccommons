...
class StatisticsController < ApplicationController
  ...
  def all_author_monthlies
    ...
    ids = blacklight_solr.find(:rows => 100000, :start => 0, :fl => "author_uni")["response"]["docs"].collect { |f| f["author_uni"] }.flatten.compact.uniq - EmailPreference.find_all_by_monthly_opt_out(true).collect(&:author)
 ...
 def detail_report
  ...
  # TODO: Should be delete with all templates. No longer in used.
  # def search_history
  ...
  #
  #       @ids = blacklight_solr.find(:per_page => 100000, :sort => "title_display asc" , :fq => @fq, :fl => 'id', :page => 1)["response"]["docs"].collect { |r| r['id'] }
  #
