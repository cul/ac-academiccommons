...
module CatalogHelper
  ...
  def refworks_export_url(document = @document)
    "http://www.refworks.com/express/expressimport.asp?vendor=#{CGI.escape(application_name)}&filter=MARC%20Format&encoding=65001&url=#{CGI.escape(solr_document_path(document.id, :format => 'refworks_marc_txt', :only_path => false))}"
  end
